...
def content_loss(content_weight, content_current, content_original):
    ...
    C = content_current.shape[1]
    content_current = content_current.squeeze().view(C, -1)
    content_original = content_original.squeeze().view(C, -1)
    content_loss = content_weight * torch.dist(content_current, content_original)**2
    return content_loss
...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.Tensor(N, C, C)
    for n in range(N):
      gram[n] = torch.matmul(features[n], torch.transpose(features[n], 0, 1))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    style_loss = 0
    for l, idx in enumerate(style_layers):
      gram = gram_matrix(feats[idx], normalize=True)
      Al = style_targets[l]
      style_loss += style_weights[l] * torch.dist(gram, Al) ** 2
...
def tv_loss(img, tv_weight):
    ...
    img3 = img[:, :, 1 : ]
    img4 = img[:, :, 0 : W - 1]
    tv_loss = tv_weight * (torch.dist(img1, img2)**2 + torch.dist(img3, img4)**2)
    return tv_loss
